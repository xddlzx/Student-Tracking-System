...
def _set_cookies(resp: Response, sid: str, csrf: str):
    resp.set_cookie(settings.SESSION_COOKIE_NAME, sid, httponly=True, samesite="lax", secure=False, path="/")
    # CSRF token is readable by JS (not HttpOnly)
    resp.set_cookie(settings.CSRF_COOKIE_NAME, csrf, httponly=False, samesite="lax", secure=False, path="/")
...
def login(payload: dict, request: Request):
    ...
    with SessionLocal() as db:
        ...
        sess = SessionModel(id=sid, user_id=user.id, role="rooter" if user.username == "rooter" else "teacher",
                            csrf_token=csrf, expires_at=new_session_expiry(),
                            user_agent=request.headers.get("user-agent"), last_seen_ip=request.client.host)
        db.add(sess); db.commit()
        resp = Response(content='{"ok":true}', media_type="application/json")
        _set_cookies(resp, sid, csrf)
        audit(db, actor_id=user.id, actor_role=sess.role, action="login", entity_type="session", entity_id=sid,
              after={"success": True})
        return resp
